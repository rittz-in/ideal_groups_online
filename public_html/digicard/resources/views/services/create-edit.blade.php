                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" name="service_name" class="form-control"
                            value="{{ old('service_name') ? old('service_name') : ($service ? $service->service_name : '') }}"
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="number" name="whatsapp_no" class="form-control"
                            value="{{ old('whatsapp_no') ? old('whatsapp_no') : ($service ? $service->whatsapp_no : '') }}"
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="file" id="logo" class="form-control" name="logo"
                            value="{{ $service ? $service->logo : '' }}">
                        @if ($service && $service->logo)
                            <img src="{{ Storage::url($service->logo) }}" alt="" width="150">
                            @if (!empty($service->logo))
                                <a class="click-service ms-5"
                                    href="{{ route('member.remove_image_service', $service->id) }}"><i
                                        class="fa-solid fa-trash "></i></a>
                            @endif
                        @endif
                        <small class="form-text text-muted fw-bold">
                            Please Upload Image Of This Height x Width : <span class="text-danger">"435 x 255"</span>
                        </small>
                        @error('logo')
                            <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                        @enderror
