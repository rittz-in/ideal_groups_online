   ...
   <script>
    $(document).ready(function() {
        $('#customersTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route('super-admin.index') }}',
            columns: [
                { data: 'name', name: 'name' },
                { data: 'email', name: 'email' },
                { data: 'created_at', name: 'created_at' },
                { data: 'card_no', name: 'card_no', visible: false }, // Hidden column for card_no
                {
                    data: 'action',
                    name: 'action',
                    orderable: false,
                ...
                {
                    data: 'card_no', // Use card_no to conditionally generate the button
                    name: 'card_no',
                    orderable: false,
                    searchable: false,
                    render: function(data, type, row) {
                        // Check if card_no is not empty
                        if (data) {
                            return '<a class="btn" href="' + '{{ route('website_url', '') }}/' + data + '" target="_blank"><img src="' + '{{ asset('assets/eye.png') }}' + '" height="25" width="25" alt="View Website"></a>';
                        }
                        return ''; // Return empty if no card_no
                    }
                },
