...
<div class="post d-flex flex-column-fluid" id="kt_post">
    <div id="kt_content_container" class="container">
        ...
        <?php
			...
			if($networks['id']!='')
            {
                $main_title = $networks['main_title'];
				$description = $networks['description'];
		        $image = $networks['image'];
                $button_name = $networks['button_name'];
				$url = $networks['url'];
                $status = ($networks['is_active'] == 'active') ? 'checked="checked"' : '';
            }
            ...
            $hidden     = array();
            echo form_open(route_to('networking_edit_url'), $attributes, $hidden);
        ?>
									...
									<label class="col-form-label fw-bold fs-6"><?=lang("Common.image")?></label>
										<div class="fv-row">
											<?php 
												...
												{ 
												?>
												<img src="<?php echo base_url().'/assets/uploads/home-section/networking/'.$image; ?>" style="max-width:100px;height:100px;" /> 
												<?php 
												}
