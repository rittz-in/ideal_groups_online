(function ($) {
    ...
    $(document).ready(function () {
		...
		/* percentage count */
		if ($('.counter-percent-wrap').length) {
            $('.counter-percent-wrap').appear(function () {
                
				//percentage count
                    var $t = $(this),
                    n = $t.find(".count-percentage").attr("data-stop"),
                    r = parseInt($t.find(".count-percentage").attr("data-speed"), 10);

                if (!$t.hasClass("counted")) {
                    $t.addClass("counted");
                    $({
                        countNum: $t.find(".count-percentage").text()
                    }).animate({
                        countNum: n
                    }, {
                        duration: r,
                        easing: "linear",
                        step: function () {
                            $t.find(".count-percentage").text(Math.floor(this.countNum));
                        },
                        complete: function () {
                            $t.find(".count-percentage").text(this.countNum);
                        }
                    });
                }
